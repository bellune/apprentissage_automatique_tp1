...
def algo_bagging(train_file,test_file):
    # 1. Charger les fichiers CSV (Assurez-vous que les chemins sont corrects)
    ...
    # 3. Créer et entraîner le modèle de Bagging avec un arbre de décision
    model = BaggingClassifier(
        base_estimator=DecisionTreeClassifier(),  # Arbre de décision comme modèle de base
        n_estimators=50,  # Nombre d'arbres à entraîner
        random_state=42,
        n_jobs=-1  # Utilise tous les cœurs du processeur pour accélérer l'entraînement
    )

    model.fit(X_train, y_train)
    print("Bagging avec arbre de décision entraîné avec succès.")
...
#2 - Bagging 
    algo_bagging(train_file,test_file)
